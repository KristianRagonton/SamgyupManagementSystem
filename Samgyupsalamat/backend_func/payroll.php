<?php
    ...
    class payroll extends sad_db{
        ...
        function add_new_payroll($employee_id,$payroll_salary,$regular_hours,$payroll_bonus,$overtime_hours,$Philhealth,$SSS,$PAGIBIG,$taxes,$total_salary,$start_date,$end_date,$other_deduc){
            $sql = "INSERT INTO payroll values ('',$employee_id,'$start_date','$end_date',0)";
            $result = $this->connect()->query($sql);
        ...
        function show_deduction_rates(){
            ...
            function get_is_samedate($floordate,$ceilingdate){
                $sql = "SELECT * FROM payroll_reports WHERE report_floor_date = '$floordate' AND report_ceiling_date='$ceilingdate'";
                $result = $this->connect()->query($sql);
                $exists = false;
                if($result->num_rows > 0){
                    $exists = true;
